...
Usage: Pls refer to https://www.livemeshthemes.com/peak/column-shortcodes/
...
if (!function_exists('mo_column_shortcode')) {
    function mo_column_shortcode($atts, $content = null, $shortcode_name = "") {
        ...
        $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="' . str_replace("_", "", $shortcode_name) . ($class ? ' ' . esc_attr($class) : '') . '"' . ($style ? ' style="' . esc_attr($style) . '"' : '') . '>' . do_shortcode(mo_remove_wpautop($content)) . '</div>';
...
if (!function_exists('mo_column_shortcode_last')) {
    ...
    function mo_column_shortcode_last($atts, $content = null, $shortcode_name = "") {
        ...
        $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="' . str_replace("_", "", str_replace('_last', ' last', $shortcode_name)) . ($class ? ' ' . esc_attr($class) : '') . '"' . ($style ? ' style="' . esc_attr($style) . '"' : '') . '>' . do_shortcode(mo_remove_wpautop($content)) . '</div><span class="clear"></span>';
...
if (!function_exists('mo_get_column_name_map')) {
    function mo_get_column_name_map() {
        $column_name_map = array(
            ...
            'one_fifth_last' => 'onefifthcol last',
...
if (!function_exists('mo_col_shortcode')) {
    function mo_col_shortcode($atts, $content = null, $shortcode_name = "") {
        ...
        $output = '<div class="' . esc_attr($class_name) . '">' . do_shortcode(mo_remove_wpautop($content)) . '</div>';
...
if (!function_exists('mo_col_shortcode_last')) {
    function mo_col_shortcode_last($atts, $content = null, $shortcode_name = "") {
        ...
        $column_map = mo_get_column_name_map();
        $class_name = $column_map[$shortcode_name];
        $output = '<div class="' . esc_attr($class_name) . '">' . do_shortcode(mo_remove_wpautop($content)) . '</div><span class="clear"></span>';
...
add_shortcode('one_fifth', 'mo_col_shortcode');
...
add_shortcode('one_fifth_last', 'mo_col_shortcode_last');
