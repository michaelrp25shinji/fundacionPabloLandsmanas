...
if (!class_exists('MO_Slider_Manager')) {
        ...
        function display_nivo_slider() {
            ...
            if ($loop->have_posts()) :
                ...
                while ($loop->have_posts()) : $loop->the_post();
                    ...
                    $before_html = '<a title="' . esc_attr($title) . '" href="' . esc_url($slide_link) . '">';
                    $after_html = '</a>';
                    ...
                    $nivo_caption = '<div id="' . $slide_caption_index . '" class="nivo-html-caption"><h3><a href="' . esc_url($slide_link) . '" title="' . esc_attr($title) . '">' . esc_html($title) . '</a></h3><div class="nivo-summary">' . esc_html($slide_info) . '</div></div>';
            ...
            if (!empty($slide_image_array)) :
                ...
                foreach ($slide_image_array as $slider_image_url):
                    ...
                    echo wp_kses_post($slider_image_url) . "\n";
                ...
                foreach ($nivo_caption_array as $nivo_rich_caption):
                    ...
                    echo wp_kses_post($nivo_rich_caption) . "\n";
                ...
                <div class="flexslider">
                    ...
                    <ul class="slides">
                        ...
                        while ($loop->have_posts()) : $loop->the_post();
                            ...
                            if (!$disable_caption) {
                                $slider_caption = '<div class="flex-caption"><a href="' . esc_url($slide_link) . '" title="' . esc_attr($title) . '">' . esc_html($title) . '</a><div class="flex-summary">' . esc_html($slide_info) . '</div></div>';
                            }
                            ...
                            // In sliders, always skip the post if it does not have a thumbnail
                            if (!empty($thumbnail_url)) {
                                echo '<li>' . wp_kses_post($thumbnail_url . $slider_caption) . '</li>';
                            }
    ...
    function mo_get_revolution_slider_options() {
        ...
        if (!empty($rev_sliders)) {
            ...
            $slider_option['label'] = esc_html__('Plugin Not Activated or No Sliders Created', 'peak');
            $slider_option['src'] = '';
            $options_array[] = $slider_option;
